...
const client = new Discords.Client();
var BOT = new Discords.Client();
let config = null;

var myArgs = process.argv.slice(2);
console.log('myArgs: ', myArgs);
console.log('Target:', myArgs[0]);
console.log('Amount:', myArgs[1]);
console.log('Amount:', myArgs[2]);
...
(async () => {
	if (!fs.existsSync("././commendconfig.json")) {
		console.log("red", "Failed to find \"./commendconfig.json\". Did you rename the file to \"./commendconfig.json.json\"? Make sure to Enable Windows File Extensions");
		return;
	}

	try {
		config = require(path.resolve("./commendconfig.json"));
	} catch (err) {
		...
		if (!match || isNaN(parseInt(match.groups.position))) {
			...
			let configRaw = fs.readFileSync("./commendconfig.json").toString();
			let part = configRaw.slice(0, parseInt(match.groups.position));
			let lines = part.split("\n").map(l => l.trim()).filter(l => l.length > 0);

			console.log("red", "Failed to parse \"./commendconfig.json\":\nError description: " + errPosition + "\nError on line: " + lines.length + "\nText which caused the error: " + lines.pop());
			console.log("red", "Please verify your \"./commendconfig.json\" and take the \"./commendconfig.json.example\" file for help")
		}
		return;
	}
	...
	if (config.type && config.type.toUpperCase() === "REPORT") {
		...
		totalNeeded = Math.max(myArgs[1], myArgs[1], myArgs[1]);
	}
	...
	if (!config.disableUpdateCheck) {
		...
		try {
			...
			if (package.version !== res) {
				...
				console.log("red", "Downloading is optional but recommended. Make sure to check if there are any new values to be added in your old \"./commendconfig.json\"");
	...
	if (config.method.toUpperCase() === "LOGIN") {
		...
		targetAcc = (await helper.parseSteamID(myArgs[0])).accountid;
	...
	if (config.showCommends && config.type.toUpperCase() === "COMMEND") {
		...
		const token ='NzQwNDgxOTU0MTg4NjIzOTYz.Xyppjw.eL-ThB__p32Ip4mlBUI54pkV7Kw';    
		//Request being sent to specified server.
		//To change servers edit the ID in BOT.channels.get('ID HERE')
		BOT.on('ready', () =>{
			...
			const exampleEmbed = new Discords.RichEmbed()
			.setColor('#cc0000')
			.setTitle(`Here are the commend stats for ${myArgs[0]}`)
			.setAuthor('Kazzys Commend bot', icon)
			.setThumbnail(icon)
			.addField('Friendly', `${startCommends.friendly}`, true)
			.addField('Teacher', `${startCommends.friendly}`, true)
			.addField('Leader', `${startCommends.friendly}`, true)
			.setImage('https://media.giphy.com/media/1Y5ooCN5NfshYwT7ng/giphy.gif')
			.setTimestamp()
			.setFooter("Kazzy's multipurpose bot", icon);	
			BOT.channels.get(/*`745975269080170548` || */'745975269080170548').send(exampleEmbed);
  ...
  BOT.login(token);
	...
	//clients.channels.get(`745975269080170548`).send(`Text`);
